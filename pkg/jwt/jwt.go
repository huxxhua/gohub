...
func NewJWT() *JWT {
	return &JWT{
		SignKey:    []byte(config.GetString("app.kty")),
		MaxRefresh: time.Duration(config.GetInt64("jwt.max_refresh_time")) * time.Minute,
	}
}

// ParserToken 解析 Token，中间件中调用
func (j *JWT) ParserToken(ctx *gin.Context) (*JWTCustomClaims, error) {

	tokenFromHeader, parseErr := j.getTokenFromHeader(ctx)
	if parseErr != nil {
		return nil, parseErr
	}

	// 1. 调用 jwt 库解析用户传参的 Token
	token, err := j.parseTokenString(tokenFromHeader)

	// 2. 解析出错
	if err != nil {
		validateErr, ok := err.(jwt.ValidationError)
		if ok {
			if validateErr.Errors == jwt.ValidationErrorMalformed {
				return nil, ErrTokenMalformed
			} else if validateErr.Errors == jwt.ValidationErrorExpired {
